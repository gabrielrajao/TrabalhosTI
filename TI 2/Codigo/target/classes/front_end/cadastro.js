...
function enviaCadastro() {
  ...
  if (validEmail && validNome && validSenha) {
    ...
    fetch("http://localhost:6789/usuario/insert?senha=" + senha + "&email=" + email + "&login=" + login, {
      ...
      .then(response => {
        ...
        if (response.ok) {
          //envia o usuario para a pagina de login para que ele possa logar (o melhor era fazer o login automatico, mas preguica)
          window.location.href = "./Login.html"
        } 
        ...
        //erro 500 sinaliza um erro interno, nao da parte do usuario
        else if (response.status == 500) {
          alert("ERRO no servidor");
        }
      })
      //promise para tratar erro do usuario
      .then(data => {
  ...
  //enquanto o usuario nao alterar os campos vermelhos, impedir envio de informações
  else {
